 ...
 *  Testimonial Widget
  */
class Blogwings_Companion_Section_Five extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'magazina-section-five',
            'description' => __('Display post in left and right column with description','blogwings-companion'));
        parent::__construct('magazina-section-five', __('Add Magazina : Post Style 5','blogwings-companion'), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        // widget content
        echo $before_widget;
        $title = isset($instance['title'])?$instance['title']:__('writing your description','blogwings-companion');
             ...
             <?php  while($one_posts->have_posts()): $one_posts->the_post();
              ?>
                ...
                <div class="post-item first">
                    ...
                    <div class="post-thumb"><a class="clickable" href="<?php the_permalink(); ?>"><?php if ((function_exists('has_post_thumbnail')) && (has_post_thumbnail())) { 
                        the_post_thumbnail( 'blogwings-companion-five-large' );
                         }
                        ?></a></div>
...
$twooption .= '<option value="'.$term_id.'" '.$selected2.'>'.$cat->name.'</option>';
  ...
  <style>
    ...
    <img src="<?php echo BLOGWINGS_COMPANION_MAGAZINA_STYLE5; ?>" />
...
<p><label for="<?php echo $this->get_field_id( 'title_bg_color' ); ?>" style="display:block;"><?php _e( 'Title Background Color:','blogwings-companion' ); ?></label> 
    ...
    <p class="thunk-widget-title">Right Blog Setting</p>
      <p>
    <label for="<?php echo $this->get_field_id('two_cate'); ?>"><?php _e('Select Specific Option To Display Post','blogwings-companion'); ?></label>
        <select name="<?php echo $this->get_field_name('two_cate'); ?>" ><?php echo $twooption; ?></select>
    </p>
     <p><label for="<?php echo $this->get_field_id('two_count'); ?>"><?php _e('Add Number Of Post To Show','blogwings-companion'); ?></label>
