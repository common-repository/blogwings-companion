 ...
 *  Add Widget
  */
class Blogwings_Companion_Section_Ads extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'magazina-section-banner',
            'description' => __('Display Banner ad or Google adsense by adding code','blogwings-companion'));
        parent::__construct('magazina-section-banner', __('Add Magazina : Ad Widget','blogwings-companion'), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        $add_code = isset($instance['add_code'])?$instance['add_code']:__('Add Ad code Here','blogwings-companion');
?>
...
<p>
        <label for="<?php echo $this->get_field_id('ad_img_uri'); ?>"><?php _e('Banner Image','blogwings-companion'); ?>
        </label>
                <?php
     ...
     <p>
        <label for="<?php echo $this->get_field_id('ads_link'); ?>"><?php _e('Banner Link','blogwings-companion'); ?></label>
    ...
    <p>
    <label for="<?php echo $this->get_field_id('add_code'); ?>"><?php _e('Google Adsense / Custom Ad','blogwings-companion'); ?></label>
    <label style="padding-top:0px;font-size: 12px;font-style: italic;"><?php _e('Generate <a target="_blank" href="https://www.google.com/adsense/start/">Google Adsense</a> code and paste it below.','blogwings-companion'); ?></label>
     <textarea  rows="8" name="<?php echo $this->get_field_name('add_code'); ?>" id="<?php echo $this->get_field_id('add_code'); ?>"  class="widefat" placeholder="<?php _e('Ad Generated Here','blogwings-companion'); ?>"><?php echo $add_code; ?></textarea>
    </p>
        <?php
    }
}
