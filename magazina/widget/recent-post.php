...
/**
 * magazina Recent Post Widget
 * This widget shows latest place post
 */
class Blogwings_Companion_RecentPost extends WP_Widget {
    /** constructor */
    function __construct() {
        $widget_ops = array(
            'classname' => 'magazina_recent_posts_widget',
            'description' => __('Display recent post in sidebar with thumbnail images','blogwings-companion')
        );
        parent::__construct('advanced-recent-posts', __('Add Magazina : Recent Post With Thumbnail','blogwings-companion'), $widget_ops);
    }
    ...
    function widget($args, $instance) {
       ...
       // echo  $new_excerpt_length = create_function('$length', "return " . $excerpt_length . ";");
...
//       echo "<pre>"; print_r($adv_recent_posts); echo "</pre>";
            ...
            <?php
             if ( $adv_recent_posts->have_posts()){ 
                ...
                <div class="th-widget-recent-post">
                      ...
                      <div class="thumb"><a href="<?php esc_url(get_permalink()); ?>"> <?php the_post_thumbnail('blogwings-companion-recent-post'); ?></a></div>
                      ...
                      <?php  } ?>
                        ...
                        echo substr(the_title($before = '', $after = '', FALSE), 0, 18) . '...'; } else {
                           ...
                           <ul class="th-recent-meta">
                            ...
                            <?php if (isset($instance['excerpt']) && $instance['excerpt']) : ?>                            
                                    <p><?php echo magazina_excerpt($excerpt_length) . $link_more; ?> </p>
                            <?php endif; ?>
                            </div>
                        <?php endif; ?>               
    ...
    function form($instance) {
        ...
        ?>
        <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title','blogwings-companion'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
        <p style="display:none;">
            <label for="<?php echo $this->get_field_id("sort_by"); ?>">
        <?php _e('Post Sort','blogwings-companion'); ?>:
                <select id="<?php echo $this->get_field_id("sort_by"); ?>" name="<?php echo $this->get_field_name("sort_by"); ?>">
                    <option value="date"<?php selected($instance["sort_by"], "date"); ?>><?php _e('Recent Sort Post','blogwings-companion'); ?></option>
                    <option value="title"<?php selected($instance["sort_by"], "title"); ?>><?php _e('Title Sort Post','blogwings-companion'); ?></option>
                    <!-- <option value="comment_count"<?php selected($instance["sort_by"], "comment_count"); ?>><php echo 'R_N_CMT'; ?></option> -->
                    <option value="rand"<?php selected($instance["sort_by"], "rand"); ?>><?php _e('Random Sort Post','blogwings-companion'); ?></option>
                </select>
            </label>
        </p>
        <p>
        ...
        <?php _e('Show Post Excerpt','blogwings-companion'); ?>
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id("excerpt_length"); ?>">
                <?php _e('Post Excerpt length','blogwings-companion'); ?>
            </label>
        ...
        <p style="display:none;">
            <label for="<?php echo $this->get_field_id("date"); ?>">
                ...
                <?php _e('Post Date','blogwings-companion'); ?> 
            </label>
        </p>
        <p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Add Number Of Post To Show','blogwings-companion'); ?></label>
