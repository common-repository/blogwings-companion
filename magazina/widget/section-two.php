 ...
 *  Testimonial Widget
  */
class Blogwings_Companion_Section_Two extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'magazina-section-two',
            'description' => __('Display featured post slider','blogwings-companion'));
        parent::__construct('magazina-section-two', __('Add Magazina : Post Style 2','blogwings-companion'), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        // widget content
        echo $before_widget;
        $title = isset($instance['title'])?$instance['title']:__('Top Stories','blogwings-companion');
...
?>
                    <div class="item">
                        <div class="post-item">
                            ...
                            the_post_thumbnail('blogwings-companion-two-small');
                            }
                            ?>
                                </div></a>
...
<?php
    ...
    function form($instance) {
        $title = isset($instance['title']) ? esc_attr($instance['title']) : __('Top Stories','blogwings-companion');
        $cate = isset($instance['cate']) ? absint($instance['cate']) : 0;
        $count = isset($instance['count']) ? absint($instance['count']) : 4;
        $title_bg_color = isset($instance['title_bg_color']) ? $instance['title_bg_color'] :"#66cda9";
         $title_txt_color = isset($instance['title_txt_color']) ? $instance['title_txt_color'] :"#fff";
...
$foption .= '<option value="'.$term_id.'" '.$selected1.'>'.$cat->name.'</option>';
    ...
    ?>
     ...
     <img src="<?php echo BLOGWINGS_COMPANION_MAGAZINA_STYLE2; ?>" />
    <p>
    ...
    <input name="<?php echo $this->get_field_name('title'); ?>" id="<?php echo $this->get_field_id('title'); ?>"  class="widefat" value="<?php echo $title; ?>" >
     ...
     <p><label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Add Number Of Post To Show','elanzalite'); ?></label>
        ...
        <p>
    <p>
    <label for="<?php echo $this->get_field_id('cate'); ?>"><?php _e('Choose Category To Show Post','blogwings-companion'); ?></label>
